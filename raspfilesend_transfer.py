...
class RaspFileSendTransfer:
    ...
    def _get_encryption_key(self):
        """Generate a machine-specific encryption key (same as config)"""
        try:
    ...
    def show_overwrite_dialog(self, filename):
        """Show custom overwrite dialog with Yes, No, Yes to All, No to All options"""
        ...
        message_frame.pack(pady=20, padx=20, fill=tk.X)
        ...
        tk.Label(text_frame, text="File already exists:", font=("Arial", 11, "bold")).pack(anchor=tk.W)
        tk.Label(text_frame, text=filename, font=("Arial", 10)).pack(anchor=tk.W, pady=(5, 0))
        tk.Label(text_frame, text="Do you want to overwrite it?", font=("Arial", 10)).pack(anchor=tk.W, pady=(5, 0))
        ...
        yes_btn = tk.Button(top_button_frame, text="Yes", width=12, 
                           command=lambda: set_result("yes"))
        yes_btn.pack(side=tk.LEFT, padx=(0, 5))
        
        no_btn = tk.Button(top_button_frame, text="No", width=12, 
                          command=lambda: set_result("no"))
        no_btn.pack(side=tk.LEFT, padx=5)
        
        cancel_btn = tk.Button(top_button_frame, text="Cancel", width=12, 
        ...
        yes_all_btn = tk.Button(bottom_button_frame, text="Yes to All", width=12, 
                               command=lambda: set_result("yes_all"))
        yes_all_btn.pack(side=tk.LEFT, padx=(0, 5))
        
        no_all_btn = tk.Button(bottom_button_frame, text="No to All", width=12, 
                              command=lambda: set_result("no_all"))
        no_all_btn.pack(side=tk.LEFT, padx=5)
        
        # Set focus and default button
        yes_btn.focus_set()
        dialog.bind('<Return>', lambda e: set_result("yes"))
        dialog.bind('<Escape>', lambda e: set_result("cancel"))
    ...
    def start_transfer(self):
        ...
        if auth_method == "password":
            ...
            if save_password and encrypted_password:
                # Use saved password
                password = self._decrypt_password(encrypted_password)
                if password:
                    self.log_message(f"🔐 Using saved password for {username}@{ip}")
                else:
                    # Decryption failed, ask for password
                    self.log_message("⚠️ Saved password could not be decrypted, please enter password")
        ...
        def transfer():
            try:
                ...
                # Transfer each file
                overwrite_all = None  # None, True (yes to all), False (no to all)
                    ...
                    try:
                        ...
                        if file_exists:
                            ...
                            if overwrite_all is True:
                                ...
                                user_choice = response[0]
                                if user_choice == "yes":
                                    should_overwrite = True
                                elif user_choice == "no":
                                    should_overwrite = False
                                elif user_choice == "yes_all":
                                    should_overwrite = True
                                    overwrite_all = True
                                    self.root.after(0, lambda: self.log_message("🔄 Will overwrite all remaining files without asking"))
                                elif user_choice == "no_all":
                                    should_overwrite = False
                                    overwrite_all = False
                                    self.root.after(0, lambda: self.log_message("⏭️ Will skip all remaining files without asking"))
                                else:  # cancel or None
                                    self.root.after(0, lambda: self.log_message("❌ Transfer cancelled by user"))
                                    sftp.close()
                                    ssh.close()
                                    return
